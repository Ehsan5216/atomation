...
import { ArrowLeftRight, CheckCircle2, AlertCircle, Package } from 'lucide-react';
...
export const StockTransferView: React.FC = () => {
  ...
  const loadBaseData = async () => {
    try {
      const [itRes, whRes, invRes, txRes] = await Promise.all([
        api.getItems(),
        api.getWarehouses(),
        api.getInventory(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(
        `انتقال یکپارچه با موفقیت ثبت شد. باقیمانده مبدا: ${formatNumber(res.sourceRemaining)} | موجودی جدید مقصد: ${formatNumber(res.destNewQuantity)}`
      );
      setQuantity('');
      setReferenceNo('');
      setNotes('');

      // Refresh inventories & transactions
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">
                  شماره برگه جابه‌جایی / حواله انتقال
                </label>
                <input
                  type="text"
                  value={referenceNo}
                  onChange={(e) => setReferenceNo(e.target.value)}
                  placeholder="TRF-1403-551"
                  className="w-full px-3 py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-slate-100 font-mono focus:outline-none focus:border-sky-500 text-xs"
                />
              </div>
            </div>
