...
import { ArrowDownToLine, CheckCircle2, AlertCircle, Package } from 'lucide-react';
...
export const StockInView: React.FC = () => {
  ...
  const loadBaseData = async () => {
    try {
      const [itRes, whRes, txRes] = await Promise.all([
        api.getItems(),
        api.getWarehouses(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(
        `ورود کالا با موفقیت ثبت شد. موجودی جدید این قفسه: ${formatNumber(res.newQuantity)} عدد`
      );
      setQuantity('');
      setReferenceNo('');
      setNotes('');
      ...
      const txRes = await api.getTransactions({ type: 'STOCK_IN', limit: 5 });
      setRecentTransactions(txRes.transactions);
    } catch (err: any) {
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">
                  شماره سند / حواله ورود / بارنامه
                </label>
                <input
                  type="text"
                  value={referenceNo}
                  onChange={(e) => setReferenceNo(e.target.value)}
                  placeholder="مثال: REC-1403-882"
                  className="w-full px-3 py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-slate-100 font-mono focus:outline-none focus:border-emerald-500 text-xs"
                />
              </div>
            </div>
