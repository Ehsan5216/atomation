...
export const Navbar: React.FC<NavbarProps> = ({ onToggleSidebar, onNavigateSettings }) => {
  const { user, logout, serverConnected, serverError, checkServerHealth } = useAuth();
  ...
  return (
    <header className="sticky top-0 z-30 bg-slate-900/95 border-b border-slate-800 backdrop-blur-md">
      ...
      {!serverConnected && (
        <div className="bg-rose-600 text-white text-xs font-semibold px-4 py-1.5 text-center flex items-center justify-center gap-2 animate-pulse shadow-inner">
          <WifiOff className="w-4 h-4" />
          <span>{serverError || 'ارتباط با سرور برقرار نیست. لطفاً اتصال اینترنت یا شبکه محلی را بررسی کنید.'}</span>
          <button
            onClick={() => checkServerHealth()}
            className="underline mr-2 text-white hover:text-amber-200 cursor-pointer font-bold"
          >
            تلاش مجدد
          </button>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        {/* Right Side: PWA Install (if available), User info, Logout */}
        <div className="flex items-center gap-2 sm:gap-3">
          {/* Connection Status Indicator */}
          {serverConnected ? (
            <div
              title="اتصال با سرور مرکزی برقرار است (کلیک برای بررسی مجدد)"
              onClick={() => checkServerHealth()}
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border bg-emerald-500/10 text-emerald-400 border-emerald-500/20 cursor-pointer hover:bg-emerald-500/20 transition"
            >
            ...
            <button
              onClick={() => checkServerHealth()}
              title="ارتباط با سرور قطع است. کلیک برای بررسی مجدد"
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border bg-rose-500/10 text-rose-400 border-rose-500/20 animate-pulse hover:bg-rose-500/20 transition cursor-pointer"
            >
