...
import { ArrowUpFromLine, CheckCircle2, AlertCircle, Package } from 'lucide-react';
...
export const StockOutView: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(
        `حواله خروج با موفقیت صادر شد. موجودی باقیمانده در این قفسه: ${formatNumber(res.remainingQuantity)} عدد`
      );
      setQuantity('');
      setReferenceNo('');
      setNotes('');
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">
                  شماره حواله / دستور کار / درخواست کالا
                </label>
                <input
                  type="text"
                  value={referenceNo}
                  onChange={(e) => setReferenceNo(e.target.value)}
                  placeholder="مثال: ISS-1403-901"
                  className="w-full px-3 py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-slate-100 font-mono focus:outline-none focus:border-rose-500 text-xs"
                />
              </div>
            </div>
