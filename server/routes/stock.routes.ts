...
import { requireAuth, requireRole, getClientIp, AuthenticatedRequest } from '../auth.js';
...
// POST /api/stock/in (ورود کالا - فقط ADMIN و WAREHOUSE_USER)
router.post('/in', requireRole(['ADMIN', 'WAREHOUSE_USER']), async (req: AuthenticatedRequest, res: Response) => {
  try {
...
// POST /api/stock/out (خروج کالا - فقط ADMIN و WAREHOUSE_USER)
router.post('/out', requireRole(['ADMIN', 'WAREHOUSE_USER']), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { itemId, warehouseId, shelfId, quantity, referenceNo, notes } = req.body;
...
// POST /api/stock/transfer (انتقال کالا - ATOMIC - فقط ADMIN و WAREHOUSE_USER)
router.post('/transfer', requireRole(['ADMIN', 'WAREHOUSE_USER']), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const {
      itemId,
      sourceWarehouseId,
...
// POST /api/stock/adjust (تراکنش اصلاحی برای اصلاح موجودی بدون حذف سابقه - فقط ADMIN و WAREHOUSE_USER)
router.post('/adjust', requireRole(['ADMIN', 'WAREHOUSE_USER']), async (req: AuthenticatedRequest, res: Response) => {
  try {
